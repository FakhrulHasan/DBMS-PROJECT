...
<?php
  ...
  if (isset($_POST["submitButton"])) {
    ...
    $s = "SELECT admissionID FROM USER WHERE admissionID = '$admissionID'";
    $result = mysqli_query($conn, $s);
    $num = mysqli_num_rows($result);


    if($num == 0){
        $query = "INSERT INTO student (first_name, last_name, gender, date_of_birth,roll,blood_group, religion,email, class, section, admissionID,phone)
        VALUES
            ('$firstname','$lastname',' $gender','$dob','$roll','$bloodgroup',' $religion','$email','$class',' $section','$admissionID','$phone')";
        ...
        echo "<script>
        alert('This admissionID  already exists in database');
        </script>";
    }
